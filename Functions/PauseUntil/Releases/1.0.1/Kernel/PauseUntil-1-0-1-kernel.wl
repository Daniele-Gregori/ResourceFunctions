...
periodicDate[time_?(Or[TimeObjectQ[#],DateObjectQ[#]]&),m_?NumericQ,unit:Alternatives@@$periodicStepsFull,opt:OptionsPattern[periodicDate]]:=
	Module[{datenew},
		datenew=DateObject[time,"Instant"](*it seems we don't need timezone option*);
		While[QuantityMagnitude@DateDifference[Now,datenew]<0,
				datenew=DatePlus[datenew,{m,unit}]];
		datenew]
